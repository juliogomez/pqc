...
> **Why is ML-DSA quantum-safe?** Like ML-KEM in the key-exchange lab, ML-DSA is *module-lattice-based*: its unforgeability rests on the Module-LWE and Module-SIS problems. The companion [module-lattices lab](../../module-lattices/README.md) builds that math from the ground up and demonstrates, with a real lattice attack, why no quantum computer can break it. Worth a detour if you want to understand the "no known quantum attack" claim rather than just trust it.
